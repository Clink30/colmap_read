 ...
 ** 此函数通过在低阶（lower octave）计算图像的高斯金字塔（Gaussian scale space），
 ** 来开始处理一张新的图像。同时，它也会清空内部的关键点缓存（keypoint buffer）。
 **
 ** @return error code. The function returns ::VL_ERR_EOF if there are
 ** no more octaves to process. 如果没有更多的octaves 处理返回ERROR
 **
...
vl_sift_process_first_octave (VlSiftFilt *f, vl_sift_pix const *im)
{
   ...
   * f->simgan.
   * 这里我们调整当前 octave（八度）中第一级的平滑程度。
   * 假设输入图像具有名义上的平滑度（模糊程度），其值为 f->simgan。
   */
  ...
  /* -----------------------------------------------------------------
   *                      Compute the first octave   计算第一个 octave
   * -------------------------------------------------------------- */
 ...
 ** angles.
 *** 该函数用于计算关键点 @a k 的一个或多个主方向。
 **  返回值为找到的方向数量（最多为 4 个）。
 **  实际的方向值会写入 @a angles 向量中。
